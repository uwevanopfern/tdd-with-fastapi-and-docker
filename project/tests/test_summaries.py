...
def test_create_summary(test_app_with_db):
    response = test_app_with_db.post(
        "/summaries/",
        data=json.dumps(
            {"url": "https://foo.bar", "summary": "Great summary, I am telling you"}
        ),
    )

    assert response.status_code == 201
    assert response.json()["url"] == "https://foo.bar"
    assert response.json()["summary"] == "Great summary, I am telling you"
...
def test_remove_summary(test_app_with_db):
    response = test_app_with_db.post(
        ...
        data=json.dumps(
            {
                "url": "https://foo.bar",
                "summary": "Another Great summary, Am telling you for sure",
            }
        ),
    )
    summary_id = response.json()["id"]
...
def test_update_summary(test_app_with_db):
    response = test_app_with_db.post(
        ...
        data=json.dumps(
            {
                "url": "https://foo.bar",
                "summary": "Great summary on updated test function",
            }
        ),
    )
...
def test_update_summary_invalid_json(test_app_with_db):
    response = test_app_with_db.post(
        "/summaries/",
        data=json.dumps({"url": "https://foo.bar", "summary": "summary_invalid_json!"}),
    )
    summary_id = response.json()["id"]
...
def test_update_summary_invalid_keys(test_app_with_db):
    response = test_app_with_db.post(
        "/summaries/",
        data=json.dumps({"url": "https://foo.bar", "summary": "summary_invalid_keys!"}),
    )
    summary_id = response.json()["id"]
